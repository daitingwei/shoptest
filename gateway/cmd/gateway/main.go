...
func init() {
	...
	flag.StringVar(&proxyConfig, "conf", "configs/config.yaml", "config path, eg: -conf config.yaml")
...
// makeDiscovery 根据 discoveryDSN 创建服务发现实例
...
func initTracing() {
	// 链路追踪由 go-kratos/gateway 的 tracing 中间件自动处理
